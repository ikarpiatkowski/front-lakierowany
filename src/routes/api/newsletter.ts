		...
		handlers: {
			POST: async ({ request }) => {
				try {
					...
					if (website) {
						console.warn(
							"Bot subscription attempt detected via honeypot:",
							email,
						);
					...
					if (MAILERLITE_GROUP_ID && subscriberId) {
						...
						if (!groupRes.ok) {
							...
							console.warn(
								"MailerLite group assign warning:",
					...
					return Response.json(
						{ error: "Wystąpił błąd podczas zapisu do newslettera." },
						{ status: 500 },
					);
				}
			},
