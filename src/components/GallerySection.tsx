...
const realizations = [
  ...
  { id: 5, title: "MDF 18mm, 36mm", type: "Dostępne grubości", img: "img5.jpg" },
  { id: 6, title: "Lakier dwustronny", type: "Lakier z każdej strony", img: "img6.jpg" },
];
        ...
        {realizations.map((item, i) => (
          ...
          >
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-brand-dark/80 via-brand-dark/10 to-transparent flex flex-col justify-end p-6">
              <span className="text-brand-primary text-xs font-bold uppercase tracking-wider mb-1">{item.type}</span>
              <h3 className="text-white text-xl font-display leading-tight">{item.title}</h3>
            </div>
          </motion.div>
        ))}
      </div>
